...
**Huidige aanpak (best effort, geen geforceerde vensters):** de "McAfee verwijderen"-instelling probeert enkel `mccleanup.exe` (indien aanwezig op de share) — dat ruimt stil op wat het kan (bv. WebAdvisor), zonder venster. Er wordt **niet** teruggevallen op de eigen uninstaller van resterende McAfee-programma's (zoals wel gebeurt bij NordVPN): op een echte pc opende die altijd een venster, en erger nog — de app bleef daarna hangen op "Bezig" wachten tot dat venster gesloten werd, ook al deed de gebruiker dat zelf. In een onbemande sessie is daar niemand om op te klikken, dus dat kan de hele sessie blokkeren. Wat na `mccleanup.exe` nog overblijft, wordt gewoon gerapporteerd in de foutmelding/log — die moet je dan handmatig afhandelen.
